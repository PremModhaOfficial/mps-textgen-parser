...
This section walks through adding a new entity to the system end-to-end.
The example is the Permissions entity — the third entity in the UserManagement service, added after User and Roles were already working.
...
You fill in the entity name, then add fields one by one using completion menus.
...
The output will be a `permissions.go` file with the same eight-step structure as `user.go` and `roles.go`, but with `Permissions`-specific type names and subjects.

[WARNING]
====
*The faithful propagation rule:* Typos in DSL field names propagate without correction.
If you name the entity `Permissons` (missing an 'i'), the generated file will be `permissons.go`, the NATS subject will be `motadata.permissons.create`, and the SQL table will be `iam.permissons`.
...
When you add a hook, the generator will:

. Include the hook stub in the generated `permissions.go` between `#HOOKS_START` and `#HOOKS_END` markers.
. Include the hook call in the `HandleCreate` method at the correct phase (pre-hooks before DAL forward, post-hooks after).
. Sort hooks by priority within each phase before generating the dispatch code.
...
* The struct fields match what you declared (correct types, correct annotations)
...
You must also add a reference to Permissions in the `SqlSchem` root.
...
This is a Go syntax error that the compiler catches immediately, but the root cause was not obvious.
...
The `field.name.capitalize()` call returned an empty string for one field.
The field name in MPS was an empty string — a field had been created in the editor but not named.
...
When you see a Go compile error that looks like a template expansion artifact (trailing dots, empty identifiers, doubled strings), trace backward to the TextGen node that produced that line — not to the Go code itself.
...
* Pre-hooks run in priority order (check `preCreateValidate` is actually called)
* DAL forwarding works (mock responder replies, main handler returns the reply)
* Post-hooks fire (check stdout for any post-hook logging)
...
* Forgetting that hook implementations in `userDefinedHooks.go` are not regenerated. If you rename a hook in MPS, the old implementation remains in `userDefinedHooks.go` under the old name, unused. The new stub is appended with an empty body. You must manually migrate and delete the old implementation.
====
