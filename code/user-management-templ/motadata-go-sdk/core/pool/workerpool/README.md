...
A high-performance, production-ready worker pool implementation with auto-scaling capabilities, backpressure support, and comprehensive concurrency management.

## Features

- ✅ **Auto-scaling worker pool** with configurable min/max workers
- ✅ **Backpressure management** with bounded queues to prevent memory exhaustion
...
- ✅ **Health check API** for Kubernetes probes
...
import (
    ...
    "motadatagosdk/core/pool/workerpool"
)

func main() {
    // Initialize the global worker pool with backpressure support
    err := workerpool.Init(workerpool.PoolConfig{
        MinWorkers:   2,
        MaxWorkers:   10,
        Timeout:      30 * time.Second,
        MaxQueueSize: 10000, // Bounded queue for backpressure
    })
    if err != nil {
        panic(err)
    }
    ...
    err = workerpool.Async(func() {
        fmt.Println("Hello from worker pool!")
    })
    
    // Handle queue full error (backpressure)
    if err == utils.ErrQueueFull {
        fmt.Println("Task queue is full, try again later")
    }
...
```go
// Initialize with custom configuration and backpressure
workerpool.Init(workerpool.PoolConfig{
    MinWorkers:   5,
    MaxWorkers:   50,
    Timeout:      time.Minute,
    MaxQueueSize: 10000,  // Bounded queue (recommended)
...
stats := workerpool.GetPoolStats()
fmt.Printf("Running: %d, Waiting: %d, Capacity: %d\n", 
    stats.Running, stats.Waiting, stats.Capacity)
fmt.Printf("Submitted: %d, Completed: %d, Failed: %d, Retried: %d\n",
    stats.Submitted, stats.Completed, stats.Failed, stats.Retried)
```

## Health Check API

```go
// Get health status for monitoring/Kubernetes probes
health := workerpool.GetHealthStatus()
if !health.Healthy {
    log.Printf("Pool unhealthy: %s", health.Reason)
}
...
// - Healthy: bool - whether pool is operational
// - Reason: string - explanation of health status
// - Running: int - currently executing tasks
// - Waiting: int - tasks in queue
// - Capacity: int - number of workers
// - Utilization: float64 - percentage of capacity in use (0.0 to 1.0)
```
...
```go
// Note: Direct pool creation uses the internal newWorkerPool function
// For most use cases, use the global pool via workerpool.Init()

// Access the global pool instance for advanced usage
...
type PoolConfig struct {
    ...
    MaxQueueSize int                  // Max queued tasks (0=unlimited, default: 10000)
    PanicHandler func(interface{})    // Custom panic handler (optional)
}
```
...
if err == utils.ErrQueueFull {
    // Queue is full, implement retry logic or rejection
    log.Println("Task queue full, applying backpressure")
}
...
### 1. Pool Lifecycle
```go
// Initialize once at application startup with backpressure
func init() {
...
// Graceful shutdown
func shutdown() {
    if err := workerpool.ShutdownWithTimeout(10 * time.Second); err != nil {
...
func monitorPool() {
    ...
    for range ticker.C {
        health := workerpool.GetHealthStatus()
        if !health.Healthy {
            alerting.SendAlert("Worker pool unhealthy: " + health.Reason)
        }
        
        if health.Utilization > 0.9 {
            log.Printf("High pool utilization: %.2f%%", health.Utilization * 100)
        }
    }
}
```
...
- **Backpressure**: Bounded queues prevent memory exhaustion under high load
...
- **Queue Management**: Configure MaxQueueSize based on your memory constraints
- **Worker Tuning**: Set MinWorkers/MaxWorkers based on CPU cores and workload
...
The package includes comprehensive test coverage with tests for:
