...
func TestTaskOptionsEdgeValues(t *testing.T) {
	...
	opts := defaultOptions()
	defaultTimeout := opts.Timeout // 30 seconds

	// Test zero and negative values - should be ignored (keep defaults)
	WithTimeout(0)(opts)
	assertions.Equal(defaultTimeout, opts.Timeout, "zero timeout should be ignored")

	WithTimeout(-time.Second)(opts)
	assertions.Equal(defaultTimeout, opts.Timeout, "negative timeout should be ignored")

	// Set a valid timeout first, then try invalid values
	WithTimeout(5 * time.Second)(opts)
	assertions.Equal(5*time.Second, opts.Timeout, "valid timeout should be applied")

	WithTimeout(-time.Second)(opts)
	assertions.Equal(5*time.Second, opts.Timeout, "negative timeout should not change existing value")

	// Test zero and negative retry values - should use safe defaults
	WithRetry(0, 0)(opts)
	assertions.Equal(0, opts.Retries, "zero retries should be 0")
	assertions.Equal(time.Second, opts.RetryDelay, "zero delay should default to 1 second")

	WithRetry(-1, -time.Second)(opts)
	assertions.Equal(0, opts.Retries, "negative retries should be treated as 0")
	assertions.Equal(time.Second, opts.RetryDelay, "negative delay should default to 1 second")
