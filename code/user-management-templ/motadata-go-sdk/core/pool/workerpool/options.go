...
// Negative or zero values are ignored (default timeout is used).
func WithTimeout(duration time.Duration) TaskOption {
	return func(taskOptions *TaskOptions) {
		if duration > 0 {
			taskOptions.Timeout = duration
		}
	}
}

// WithRetry configures retry behavior for task failures.
// retries: number of retry attempts (0 = no retries, negative values are treated as 0)
// delay: base delay between retries (negative values are treated as 1 second)
func WithRetry(retries int, delay time.Duration) TaskOption {
	return func(taskOptions *TaskOptions) {
		if retries > 0 {
			taskOptions.Retries = retries
		} else {
			taskOptions.Retries = 0
		}
		if delay > 0 {
			taskOptions.RetryDelay = delay
		} else {
			taskOptions.RetryDelay = time.Second
		}
	}
}
