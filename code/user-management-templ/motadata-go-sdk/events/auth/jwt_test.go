...
var (
	testTenantJWT   = "tenant-123"
	testSubject     = "test-subject"
	testIssuer      = "test-issuer"
	testUserName    = "test-user"
	testTokenSimple = "token"
)
...
func TestParseJWT(t *testing.T) {
	...
	assertions.Equal(testSubject, parsedClaims.Subject, "Subject should match")
	assertions.Equal(testIssuer, parsedClaims.Issuer, "Issuer should match")
	assertions.Equal(testUserName, parsedClaims.Name, "Name should match")
}
...
func TestNewJWTCredential(t *testing.T) {
	...
	cred, err := NewJWTCredential(testTenantJWT, jwt, testSeedValue)

	assertions.NoError(err, "Should create credential")
	assertions.NotNil(cred, "Credential should not be nil")
	assertions.Equal(testTenantJWT, cred.TenantID, "TenantID should match")
	assertions.Equal(jwt, cred.JWT, "JWT should match")
	assertions.Equal(testSeedValue, cred.Seed, "Seed should match")
...
func TestNewJWTCredentialInvalidJWT(t *testing.T) {
	...
	cred, err := NewJWTCredential(testTenantJWT, "invalid", testSeedLiteral)
...
func TestNewJWTCredentialExpiredJWT(t *testing.T) {
	...
	cred, err := NewJWTCredential(testTenantJWT, jwt, testSeedLiteral)
...
func TestJWTCredentialToCredentials(t *testing.T) {
	...
	assertions.Equal(testJWTTokenLiteral, genericCreds.JWT, "JWT should match")
	assertions.Equal(testSeedValue, genericCreds.Seed, "Seed should match")
}
