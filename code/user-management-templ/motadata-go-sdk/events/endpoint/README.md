...
### Step 3: Set Up Distributed Handlers

```go
handler := endpoint.NewHandler(manager)
handler.RegisterQueueSubscriptions(ctx, sub, "endpoint-workers")
```
...
go func() {
    ...
    for range ticker.C {
        client := endpoint.NewClient(pub)
        client.Renew(ctx, "user-api-v1", 5*time.Minute)
    }
}()
```

### Step 6: Monitor Health Changes
...
```go
manager.DeregisterEndpoint(ctx, "user-api-v1")
manager.Shutdown(ctx)
```
