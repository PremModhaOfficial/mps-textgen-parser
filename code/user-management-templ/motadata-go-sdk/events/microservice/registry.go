...
func (r *Registry) RegisterInstance(instance *Instance) error {
	...
	if instExists {
		r.updateExistingInstance(service, instance, existingInst, now)
	} else {
		r.registerNewInstance(service, instance, now)
	}

	return nil
}

func (r *Registry) updateExistingInstance(service *Info, instance, existing *Instance, now time.Time) {
...
func (r *Registry) registerNewInstance(service *Info, instance *Instance, now time.Time) {
...
// removeFromStringIndex removes an ID from a string-keyed index map and cleans up the key if empty.
func removeFromStringIndex(index map[string]map[string]struct{}, key, id string) {
	if key == "" {
		return
	}
...
// removeFromTypeIndex removes an ID from a ServiceType-keyed index map and cleans up the key if empty.
func removeFromTypeIndex(index map[ServiceType]map[string]struct{}, key ServiceType, id string) {
	if key == "" {
		return
	}
	entries, ok := index[key]
...
func (r *Registry) removeFromIndicesLocked(svc *Info) {
	removeFromStringIndex(r.byName, svc.Name, svc.ID)
	removeFromTypeIndex(r.byType, svc.Type, svc.ID)

	for _, cap := range svc.Capabilities {
		removeFromStringIndex(r.byCapability, cap, svc.ID)
	}
	for _, tag := range svc.Tags {
		removeFromStringIndex(r.byTag, tag, svc.ID)
	}
}
