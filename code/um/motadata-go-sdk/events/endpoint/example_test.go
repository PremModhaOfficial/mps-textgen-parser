...
func Example_callbacks() {
	...
	// Update status
	registry.UpdateStatus("callback-test", endpoint.StatusDegraded)

	// Wait for status change callback
	fmt.Printf("Status change: %s\n", <-statusChanged)

	// Deregister
	registry.DeregisterWithReason("callback-test", "test complete")
...
func Example_manager() {
	...
	endpoints := manager.Discover("my-service",
