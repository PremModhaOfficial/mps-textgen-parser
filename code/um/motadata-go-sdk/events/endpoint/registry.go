...
func (r *Registry) removeFromIndicesLocked(ep *Info) {
	...
	if ep.ServiceID != "" {
		if svc, ok := r.byService[ep.ServiceID]; ok {
			delete(svc, ep.ID)
			if len(svc) == 0 {
				delete(r.byService, ep.ServiceID)
			}
		}
	}
	...
	if ep.InstanceID != "" {
		if inst, ok := r.byInstance[ep.InstanceID]; ok {
			delete(inst, ep.ID)
			if len(inst) == 0 {
				delete(r.byInstance, ep.InstanceID)
			}
		}
	}
	...
	if ep.Host != "" {
		if host, ok := r.byHost[ep.Host]; ok {
			delete(host, ep.ID)
			if len(host) == 0 {
				delete(r.byHost, ep.Host)
			}
		}
	}
