...
// Used internally by batch operations (deprecated in favor of direct error arrays).
type BatchResult struct {
	Index int   // Index of the function in the batch
	error error // error returned by the function (nil if successful)
}
...
func Batch(ctx context.Context, fns ...func() error) []error {
	...
	case <-done:
		return errs
	case <-ctx.Done():
		batchCancel() // Cancel all remaining tasks
